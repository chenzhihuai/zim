return {

  'mrjones2014/legendary.nvim',
  -- since legendary.nvim handles all your keymaps/commands,
  -- its recommended to load legendary.nvim before other plugins
  priority = 10000,
  lazy = false,
  -- sqlite is only needed if you want to use frecency sorting
  -- dependencies = { 'kkharji/sqlite.lua' }
  dependencies = { 'stevearc/dressing.nvim' },
  keys = {
    { "<c-s-p>",   "<cmd>Legendary<cr>", "Command Palette" },
    { "<leader>?", "<cmd>Legendary<cr>", "Command Palette" },
  },
  opts = {
    keymaps = {
      -- NOTE: Telescope

      -- NOTE: lsp
      { 'K',          description = 'Open an LSP hover window above cursor' },
      { ']d',         description = 'Next Diagnostic' },
      { '[d',         description = 'Previous Diagnostic' },
      { 'gd',         description = 'Goto Definition' },
      { 'gr',         description = 'Goto Reference' },
      { 'gI',         description = 'Goto Implementation' },
      { '<leader>D',  description = 'Type Definition' },
      { '<leader>rn', description = 'Rename Symbol' },
      { '<leader>ca', description = 'Code Action' },
      { 'gD',         description = 'Open the declaration' },
      -- NOTE: Terminal
      { '<leader>tt', description = 'Terminal Float' },
      { '<leader>tf', description = 'Terminal Float' },
      { '<leader>tb', description = 'Terminal Bottom' },
      { '<leader>?',  description = 'Legendary' },
      -- NOTE: Tab/Buffer
      { '[b',         description = 'Previous Buffer' },
      { ']b',         description = 'Next Buffer' },
      { 'Q',          description = 'Close Buffer' },
      -- NOTE: Git
      { '<leader>gg', description = 'Interactive Git' },
      -- { '<leader>gl', description = 'Git' },
      -- NOTE: Session
      { '<leader>ss', description = 'Save Session' },
      { '<leader>sl', description = 'Load Session' },
    },
    commands = {
      -- easily create user commands
      -- {'Format', vim.lsp.buf.format, description = 'Format whole buffer' },
      { 'Format',  description = 'Format whole buffer' },
      { 'Neotree', description = "File Explorer" },
      { 'Neogit',  description = 'Interactive Git Interface' },
    },
    funcs = {
      -- Make arbitrary Lua functions that can be executed via the item finder
      -- { vim.lsp.buf.definition, description = 'goto definition' },
      -- { vim.lsp.buf.references, description = 'goto references' },
      -- { vim.lsp.buf.rename, description = 'rename symbol' },
    },
    autocmds = {
      -- Create autocmds and augroups
      -- { 'BufWritePre', vim.lsp.buf.format, description = 'Format on save' },
      {
        name = 'MyAugroup',
        clear = true,
        -- autocmds here
      },
    },
    -- load extensions
    extensions = {
      -- automatically load keymaps from lazy.nvim's `keys` option
      lazy_nvim = true,
      -- load keymaps and commands from nvim-tree.lua
      -- nvim_tree = true,
      -- load commands from smart-splits.nvim
      -- and create keymaps, see :h legendary-extensions-smart-splits.nvim
      -- smart_splits = {
      --   directions = { 'h', 'j', 'k', 'l' },
      --   mods = {
      --     move = '<C>',
      --     resize = '<M>',
      --   },
      -- },
      -- load commands from op.nvim
      -- op_nvim = true,
      -- load keymaps from diffview.nvim
      diffview = true,
    },
    sort = {
      -- put most recently selected item first, this works
      -- both within global and item group lists
      most_recent_first = true,
      -- sort user-defined items before built-in items
      user_items_first = true,
      -- sort the specified item type before other item types,
      -- value must be one of: 'keymap', 'command', 'autocmd', 'group', nil
      item_type_bias = nil,
      -- settings for frecency sorting.
      -- https://en.wikipedia.org/wiki/Frecency
      -- Set `frecency = false` to disable.
      -- this feature requires sqlite.lua (https://github.com/kkharji/sqlite.lua)
      -- and will be automatically disabled if sqlite is not available.
      -- NOTE: THIS TAKES PRECEDENCE OVER OTHER SORT OPTIONS!
      -- frecency = {
      -- 	-- the directory to store the database in
      -- 	db_root = string.format('%s/legendary/', vim.fn.stdpath('data')),
      -- 	-- the maximum number of timestamps for a single item
      -- 	-- to store in the database
      -- 	max_timestamps = 10,
      -- },
    },
  }
}
